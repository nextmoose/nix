{ config, pkgs, ... } : let
    ...
    structure-cat = value : file-name : name : "--run \"export ${ utils.upper-case name }=${ builtins.replaceStrings [ "\"" "\$" ] [ "\\\"" "\\\$" ] ( builtins.concatStringsSep "" [ "\$( " pkgs.coreutils "/bin/cat " "\$( " ( builtins.toString value ) "/bin/structure )" "/" file-name " )" ] ) }\"" ;
   utils = {
	...
	single-site-dot-ssh = name : host : host-name : user : port : identity-file : user-known-hosts-file : utils.sh-derivation name { host = host ; host-name = host-name ; user = user ; port = port ; identity-file = identity-file ; } [ pkgs.coreutils pkgs.gnused ] ;
...
#	    ( ( structures "/home/user/structures" ).ssh-keygen ( structure-cat ( ( structures "/home/user/structures" ).personal-identification-number ( literal "pin.asc" ) ( literal 6 ) ( literal "67b4e892-ef69-4253-9e21-459a1c33645a" ) ) "pin.asc" ) )
	    ( ( structures "/home/user/structures" ).single-site-dot-ssh ( literal "upstream" ) ( literal "github.com" ) ( literal "git" ) ( literal 22 ) ( structure-file ( ( structures "/home/user/structures" ).ssh-keygen ( structure-cat ( ( structures "/home/user/structures" ).personal-identification-number ( literal "pin.asc" ) ( literal 6 ) ( literal "67b4e892-ef69-4253-9e21-459a1c33645a" ) ) "pin.asc" ) ) "id-rsa.asc" ) ( structure-file ( utils.system-secret ( literal "upstream.known_hosts" ) ( literal "known-hosts.asc" ) ) "known-hosts.asc" ) )
