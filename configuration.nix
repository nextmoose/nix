{ config, pkgs, ... } : let
	...
	pass = name : executable-name : dot-gnupg : password-store-dir : pkgs.stdenv.mkDerivation {
	    ...
	    installPhase = "makeWrapper ${ pkgs.pass }/bin/pass $out/bin/${ executable-name } ${ dot-gnupg "dot-gnupg" } ${ password-store-dir "password-store-dir" } --run 'export PASSWORD_STORE_GPG_OPTS=\"--pinentry ask --homedir \$DOT_GNUPG\"' --set PATH ${ pkgs.lib.makeBinPath [ pkgs.pinentry pkgs.pinentry-qt ] }" ;
	} ;
