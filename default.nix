...
EOF
	...
	rebuild-nixos = name : utils.sh-derivation name { } [ ] [ pkgs.coreutils pkgs.gnugrep pkgs.rsync pkgs.systemd ] ;
        s3fs = name : home : password-file : bucket : utils.sh-derivation name { home = home ; password-file = password-file ; bucket = bucket ; } [ ] [ pkgs.awscli pkgs.s3fs ] ;
	...
	personal-identification-number = digits : uuid : utils.structure "${ derivations.personal-identification-number digits uuid }/bin/personal-identification-number" { } ;
        s3fs = home : password-file : bucket : utils.structure "${ derivations.s3fs home password-file bucket }/bin/s3fs" { } ;
...
in {
	...
	s3fs-password = structures.s3fs-password aws.aws-access-key-id aws.aws-secret-access-key ;
        s3fs = structures.s3fs ( structure-dir aws-configure ) ( structure-file "password-file.asc" s3fs-password ) ( literal "de910e42-98cb-4a4c-998b-b570c5bd687f" ) ;
