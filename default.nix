...
EOF
	...
	personal-identification-number = file-name : digits : uuid : utils.structure "${ derivations.personal-identification-number file-name digits uuid }/bin/personal-identification-number" { } ;
    } ;
in {
    ...
    shell = let
        report-pin = structures.personal-identification-number ( literal "pin.asc" ) ( literal 6 ) ( literal "bac2c05d-1668-4dd9-9d6e-8729d7673811" ) ;
    ...
    in pkgs.mkShell {
	    ...
	    export REPORT_PIN=$( ${ pkgs.coreutils }/bin/cat $( ${ report-pin }/bin/structure )/pin.asc ) &&
	        export REPORT_PIN2=$( ${ report-pin }/bin/structure ) &&
