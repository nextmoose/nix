...
EOF
	...
	rebuild-nixos = name : utils.sh-derivation name { } [ ] [ pkgs.coreutils pkgs.gnugrep pkgs.rsync pkgs.systemd ] ;
        s3fs = name : aws-access-key-id : aws-secret-access-key : aws-default-region : bucket : cache-dir : utils.sh-derivation name { password-file = structure-file "password-file.asc" ( structures.s3fs-password aws-access-key-id aws-secret-access-key ) ; aws-access-key-id = aws-access-key-id ; aws-secret-access-key = aws-secret-access-key ; aws-default-region = aws-default-region ; bucket = bucket ; cache-dir = cache-dir ; } [ ] [ pkgs.awscli pkgs.s3fs ] ;
	...
	personal-identification-number = digits : uuid : utils.structure "${ derivations.personal-identification-number digits uuid }/bin/personal-identification-number" { } ;
        s3fs = aws-access-key-id : aws-secret-access-key : aws-default-region : bucket : cache-dir : utils.structure "${ derivations.s3fs aws-access-key-id aws-secret-access-key aws-default-region bucket cache-dir }/bin/s3fs" { } ;
...
in {
	...
	s3fs-password = structures.s3fs-password aws.aws-access-key-id aws.aws-secret-access-key ;
        s3fs = structures.s3fs aws.aws-access-key-id aws.aws-secret-access-key aws.aws-default-region ( literal "de910e42-98cb-4a4c-998b-b570c5bd687f" ) ( structure-dir ( structures.temporary "52a855ff-06f5-4264-a604-75e36c31ab2a" ) ) ;
	...
	secrets = [
	    ...
	    ( derivations.pass "s3-secrets" ( structure-dir ( structures.dot-gnupg ( structure-file "secret.asc" boot.gpg-private-keys ) ( structure-file "secret.asc" boot.gpg-ownertrust ) ( structure-file "secret.asc" boot.gpg2-private-keys ) ( structure-file "secret.asc" boot.gpg2-ownertrust ) ) ) ( structure-dir ( structures.git-project ( literal "${ ssh }/bin/ssh" ) ( literal "${ derivations.post-commit ( literal "personal" ) }/bin/post-commit" ) ( literal "Emory Merryman" ) ( literal "emory.merryman@gmail.com" ) ( literal "upstream:nextmoose/secrets.git" ) ( literal "personal:nextmoose/secrets.git" ) ( literal "report:nextmoose/secrets.git" ) ( literal "78dcfe85-a027-44f9-b10b-e92a85a3b716" ) ) ) { kludge-pinentry = { program = "${ derivations.pass-kludge-pinentry }/bin/pass-kludge-pinentry" ; completion = "${ derivations.pass-kludge-pinentry }/src/completion.sh" ; } ; kludge-start = { program = "${ derivations.pass-kludge-start ( literal "ece43b24-7eb0-4a8e-a892-5a368489c7f0" ) ( literal "78dcfe85-a027-44f9-b10b-e92a85a3b716" ) }/bin/pass-kludge-start" ; completion = "${ derivations.pass-kludge-start ( literal "ece43b24-7eb0-4a8e-a892-5a368489c7f0" ) ( literal "78dcfe85-a027-44f9-b10b-e92a85a3b716" ) }/src/completion.sh" ; } ; expiry = { program = "${ derivations.pass-expiry }/bin/pass-expiry" ; completion = "${ derivations.pass-expiry }/src/completion.sh " ; } ; } )
